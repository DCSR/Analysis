...
class myGUI(object):
    def __init__(self):
        """
        This object controls all aspects of the Graphical User Intervace:
        ...
        loadZimmmerButton = Button(headerFrame, text="Ben's data", command= lambda: \
                              self.openWakeFile("Bens TH_H419_Apr_15.str")).grid(row=0,column=4,sticky=N)
        ...
        # *********  Neumaier Lab stuff **************
        self.boxNum = StringVar(value="All")
        neumaierFrame = Frame(self.columnFrame, borderwidth=2, relief="sunken")
        neumaierFrame.grid(column = 0, row = 3)
        neumaierFrameLabel = Label(neumaierFrame, text = "Neumaier Stuff").grid(row = 0, column=0)
        
        loadAllButton = Button(neumaierFrame, text="Load", command= lambda: \
                              self.loadWashUFile(1,self.boxNum.get())).grid(row=1,column=0,sticky=N)
        #loadSelectedButton = Button(neumaierFrame, text="Load Box", command= lambda: \
        #                      self.neumaierTest(self.boxNum.get())).grid(row=2,column=0,sticky=N)
        options = ['All','1','2','3','4','5','6','7','8']
        selectBox = OptionMenu(neumaierFrame,self.boxNum,options[0],*options)
        selectBox.grid(row=1, column = 1)         
        dose1Button = Radiobutton(neumaierFrame, text="0.1688 mg/inj", variable=self.neumaierDose, value=1)
        dose1Button.grid(row=3, column = 0)
        dose2Button = Radiobutton(neumaierFrame, text="0.3375 mg/inj", variable=self.neumaierDose, value=2)
        dose2Button.grid(row=4, column = 0)
        dose3Button = Radiobutton(neumaierFrame, text="0.675 mg/inj", variable=self.neumaierDose, value=3)
        dose3Button.grid(row=5, column = 0)
        # TO DO:
        # add average button
        # print out various stuff 
        
        # ********* Ferguson Lab stuff **************    
        fergusonFrame = Frame(self.columnFrame, borderwidth=2, relief="sunken")
        fergusonFrame.grid(column = 0, row = 4)
        fergusonFrameLabel = Label(fergusonFrame, text = "Ferguson Stuff").grid(row = 0, column=0)
        loadFergusonButton = Button(fergusonFrame, text="Load Ferguson File", command= lambda: \
                              self.loadFergusonFile(self.boxNum.get())).grid(row=1,column=0,sticky=N)
    ...
    def openFile(self, fileName):
        ...
        # MUSCPumpTimes = [8.175,4.597,2.585,1.454,0.818,0.460,0.259,0.145,0.082,0.046,0.026,0.0146]
        # Samaha: cocConc = 0.75 mg/ml
        # pumpSpeed defaults to 0.143 mls/5 sec = 0.0286 ml/sec
        ...
        if len(fileName) > 0:
            ...
            if fileName.find(".str") > 0:
                ...
                self.recordList[selected].TH_PumpTimes = WakePumpTimes
                ...
                aFile.close()
                self.recordList[selected].cocConc = 5.0
                self.recordList[selected].pumpSpeed = 0.025 # Wake default
                self.recordList[selected].TH_PumpTimes = WakePumpTimes
            elif fileName.find(".txt") > 0:             
                self.recordList[selected].dataList = Samaha.translateSamahaFile(fileName)
                self.recordList[selected].cocConc = 0.75    # mg/ml
                self.recordList[selected].pumpSpeed = 0.0286 # ml/sec
                self.recordList[selected].TH_PumpTimes = WakePumpTimes
    ...
    def loadFergusonFile(self, boxNum):
        """
        Loads a selected file using the format used by Susan Ferguson, University of Washington.
        Characteristics and assumptions about the Med Associates file type. (Note that the format
        is quite different from that used by the Neumaier lab).

        - The procedure can grab information on up to ten boxes; seven boxes appears to be typical.

        - Info grabbed from the header:
          Date
          Subject no.
          Box number
          Start time
        Could be used for error checking:
          F: Typically = 1; file says: "FR countdown variable for right"
          G: number of timestamps; should equal the number of entries in lists D: and E:
          H: duration?  Check whether this corresponds to final timestamp in D:
          U: Typically = 1; files says: "FR countdown variable for left"
          X: Typically = 1; "fixed ratio"
          Y: file syas "number of reinforcers (infusions)"
          A: always has 50 entries; don't know what these represent 
          B: list of progressive ratios: These do not appear to fit a mathmatically defined
             progression, inplying that the breakpoint is not a scaler variable.
             5,10,20,30,45,65,85,115,145,185,225,275,325,385,445,515,585,
             665,745,835,925,1025,1125,1235,1345,1465
          C: 10 entries, typically zeros; don't know what these are used for

        Most important sections of datafile:
          D: List of timestamps
          E: Codes representing the event that occured at the corresponding timestamp in D:
             See bleow for the breakdown of these codes.
        """
        print("Loading Ferguson File type\n")
        pathAndFilename = filedialog.askopenfilename()
        #print(pathAndFilename)
        #pathAndFilename = "/Users/daveroberts/Desktop/Ferguson/Day_14_SA.txt"       
        directory = os.path.split(pathAndFilename)[0]
        fileName = os.path.split(pathAndFilename)[1]
        ...
        subjectID = ""
        boxNum = 0
        # BoxList is "zero indexed", i.e. BoxList[0] is tempBox1List
        boxList = [tempBox1List,tempBox2List,tempBox3List,tempBox4List,tempBox5List,tempBox6List,tempBox7List,tempBox8List,tempBox9List,tempBox10List]
        noteList = [tempnote1,tempnote2,tempnote3,tempnote4,tempnote5,tempnote6,tempnote7,tempnote8,tempnote9,tempnote10]
        timeStampSectionIdentified = False
        codeSectionIdentified = False
        listIndex = -1
        dataPointIndex = 0
        with open(pathAndFilename) as myOpenFile:
            for line in myOpenFile:            # This runs through the file one line at a time
                lineNumber = lineNumber + 1    # count the lines
                if "Start Date" in line:
                    """
                    This signals the start of a section for a different box.
                    The code parses the data in temporary lists (boxList and noteList) until
                    all the data are read in. Then after the file is closed it is potentially checked (?)
                    and transferred to the main program record list.
                    """
                    listIndex = listIndex + 1

                    textList = line.split()    # textList will look like ['Start', 'Date:', '07/04/15']
                    date = textList[2]         # Zero indexing will refer to the third elemant as 2
                elif "Subject" in line:
                    textList = line.split()    # textList will look like ['Subject:', '385']
                    subjectID = textList[1]    # Zero indexing will refer to the second element as 1
                elif "Box:" in line:
                    textList = line.split()
                    boxNum = textList[1]       # Note that, here, boxNum is a string not an integer
                    noteList[listIndex] = "Subject "+subjectID+" run in Box "+boxNum+" on "+date
                elif "D:" in line:
                    timeStampSectionIdentified = True   # if it finds "D:" then it knows that it is in the right section of the datafile
                    codeSectionIdentified = False
                    # listIndex = listIndex + 1
                    # print("TimeStamp Section for Box ", listIndex, "begins at line ", lineNumber)
                elif "E:"in line:  
                    timeStampSectionIdentified = False
                    codeSectionIdentified = True
                    # print("Code Section for Box ", listIndex, "begins at line ", lineNumber)
                    dataPointIndex = 0
                elif (timeStampSectionIdentified):
                    if line[0] == " ":      # looks for an indented section of text by checking that the first character is a space.
                        numberList = line.split()                 # splits the line up into a list of numbers (as text)  
                        for dataNumber in numberList:             # goes through each number in the list from each line
                            if dataNumber.find(".") > 0:          # ignores a number that doesn't have a decimal point, i.e. 0: 5: 10: etc
                                time = int(float(dataNumber))*10  # converts the textual representation of a float to an integer
                                pair = [time,0]                   # create a "undefined" timestamp 
                                boxList[listIndex].append(pair)                      
                elif (codeSectionIdentified):
                    if line[0] == " ":      # looks for an indented section of text by checking that the first character is a space.
                        numberList = line.split()               # splits the line up into a list of numbers   
                        for dataNumber in numberList:           # goes through each number in the list from each line
                            if dataNumber.find(".") > 0:        # ignores a number that doesn't have a decimal point, i.e. 0: 5: 10: etc
                                code = int(float(dataNumber))              
                                boxList[listIndex][dataPointIndex][1] = code
                                dataPointIndex = dataPointIndex + 1
                                if dataPointIndex == len(boxList[listIndex]):    # Exit code section 
                                    codeSectionIdentified = False                                                                                
            myOpenFile.close()

            """
            SelfAdmin099.py Box Object:
                boxNum = boxNum        
                subject_ID_string = 'box'+str(boxNum)
                weight = 330
                schedNum = 0      # To Do: define schedules such as 0 = "do not run", 1 = FR etc.
                dose = 1.0
                dataList = []

            Datalist timestamp codes: 
            ...
            Additional codes to accommodate Ferguson Data
            H, h = Houselight on/off
            C, c = Second Cue light on/off
            >    = Lever 1 inactive responses
            <    = Lever 2 inactive responses
            F    = Food tray
            
            The following code runs through every timestamp pair and swaps out the Ferguson
            number code with a Roberts letter code.

            Good but not necessaril perfect match.
            """

            for boxIndex in range(10):
                for pair in boxList[boxIndex]:
                    if pair[1] == 3: pair[1] = "L"       # Right active FR press
                    elif pair[1] == 4: pair[1] = "J"     # Left active FR press            
                    elif pair[1] == 5: pair[1] = "P"     # Infusion start
                    elif pair[1] == 6: pair[1] = "p"     # Infusion end
                    elif pair[1] == 7: pair[1] = "F"     # Food tray head entry
                    elif pair[1] == 8: pair[1] = ">"     # Right inactive press during FR
                    elif pair[1] == 9: pair[1] = "<"     # Left inactive press during FR
                    elif pair[1] == 10: pair[1] = "="    # Right lever extends
                    elif pair[1] == 11: pair[1] = "-"    # Left lever extends
                    elif pair[1] == 12: pair[1] = "."    # Right lever retracts
                    elif pair[1] == 13: pair[1] = ","    # Left lever retracts
                    elif pair[1] == 34: pair[1] = ">"    # Right press during infusion and time-out period
                    elif pair[1] == 35: pair[1] = "<"    # Left press during infusion and time-out period
                    elif pair[1] == 36: pair[1] = ">"    # Right press in no-drug period
                    elif pair[1] == 37: pair[1] = "<"    # Left press in no-drug period
                    elif pair[1] == 38: pair[1] = "S"    # Right lever cue light on
                    elif pair[1] == 39: pair[1] = "C"    # Left lever cue light on
                    elif pair[1] == 40: pair[1] = "s"    # Right lever cue light off
                    elif pair[1] == 41: pair[1] = "c"    # Left lever cue light off
                    elif pair[1] == 42: pair[1] = "H"    # Houselight on
                    elif pair[1] == 43: pair[1] = "h"    # Houselight off
                    elif pair[1] == 44: pair[1] = "B"    # Drug period starts
                    elif pair[1] == 45: pair[1] = "b"    # No-drug period starts
                    ...
                    elif pair[1] == 100: pair[1] = "E"   # End session marker
                    else: pair[1] = "u"


                # 46,47 and 49 used in the cue file

                self.recordList[boxIndex].recordListIndex = [] # empty the main program list
                name = fileName+":Box "+ str(boxIndex+1)
                self.recordList[boxIndex].fileName = name
                self.recordList[boxIndex].notes = noteList[boxIndex]
                self.fileNameList[boxIndex].set(name)
                self.recordList[boxIndex].cocConc = 3.2   # mg/ml
                self.recordList[boxIndex].pumpSpeed = 0.0178 # ml/sec            
                self.recordList[boxIndex].dataList = boxList[boxIndex]
                self.recordList[boxIndex].extractStatsFromList()
                print(self.recordList[boxIndex])

                #  Amanda says:
                # 1. volume of injection: 50 uL (0.05 mL) 
                # 2. length of injection: 2.8 s (and yes, you were correct on that)
                # 3. dose of a single injection: 0.4 mg/kg/infusion
                # So, 50 ul/2.8 s = 17.857 ul/sec = 0.0178 ml/sec
                # 0.4 mg/kg/inj ~ 1 mg/inj for a 400 gram rat
                # 0.16ug / 50 ul =  mg/ml = 3.2 mg/ml
                # 0.16 mg/inj = 0.4 mg/kg/inj (400 gram rat)
                

            """
            print("Length of timeStampList for Box ", i+1, len(boxList[i]))
            print(boxList[0])
            print("Box 1,fifth tuple =",boxList[0][4])
            boxList[0][4][1] = "u"
            print("Box 1,fifth tuple =",boxList[0][4])
            """       


    def loadWashUFile(self,fileFormat,boxNum):
        """ Opens a Washington University Med Associates datafile and returns a list of datapoints.
        fileFormat: 1 = Neumaier, 2 = Ferguson
        The Neurmaier datapoints are extracted from a section of the datafile
        that is preceded by the section identifier "H:" and ends with "K:"
        """
        verbose = False    # Used for debugging. Change to "True" if you want to see all the print statements.
        if verbose:
            print (" *********** Entering neumaierCode procedure *************")
            print ("Operating system:", os.name)
            print ("Python version:")
            print(sys.version)
            print("tkinter version:", TkVersion)
        # default to Neumaier format
        sectionStartIdentfier = "H:"
        sectionEndIdentifier = "K:"
        if fileFormat == 2:   # Ferguson format
            sectionStartIdentfier = "D:"
            sectionEndIdentifier = "E:"
        self.clearCanvas()
        #fileName = "NeumaierTest.txt"
        tempName = filedialog.askopenfilename()
        directory = os.path.split(tempName)[0]
        fileName = os.path.split(tempName)[1]

        # This section loads all available data from the file into temp lists 
        ...
        tempBox8List = []
        # BoxList is "zero indexed", i.e. BoxList[0] is tempBox1List
        boxList = [tempBox1List,tempBox2List,tempBox3List,tempBox4List,tempBox5List,tempBox6List,tempBox7List,tempBox8List]
        # sectionIdentifier = "H:"
        if verbose: print("Looking for section preceded by "+sectionStartIdentfier+" in "+fileName+"\n")
        # print(tempString)
        dataPoints = 0
        lineNumber = 0
        ...
        with open(fileName) as myOpenFile:
            for line in myOpenFile:            # This runs through the file one line at a time
                lineNumber = lineNumber + 1            # count the lines
                if sectionStartIdentfier in line:
                    sectionIdentified = True           # if it finds "H:" then it knows that it is in the right section of the datafile
                    listIndex = listIndex + 1
                    if verbose: print("Box", listIndex + 1,"data list begins at line ", lineNumber)
                elif (sectionIdentified):
                    if line[0] == " ":      # looks for an indented section of text by checking that the first character is a space. 
                        numberList = line.split()               # splits the line up into a list of numbers
                        for dataNumber in numberList:           # goes through each number
                            if dataNumber.find(".") > 0:        # ignore a number that doesn't have a decimal point
                               data = float(dataNumber)
                               if data > 0:                     # ignore zero values
                                   if listIndex <= 7:
                                       boxList[listIndex].append(data)
                                       dataPoints = dataPoints + 1
                    # if the line doesn't begin with " " then it is at the end of the indented section                       
                    elif sectionEndIdentifier in line:  # i.e. "K:" or "E:"
                        sectionIdentified = False
            myOpenFile.close()
        if verbose:
            for index in range(7):
                print("Box", index+1,"list contains", len(boxList[index]), "data points")
                print(boxList[index])
        print ("Lines in datafile =", lineNumber)
        self.graphCanvas.create_text(100,10,text = fileName)

        def createDataFile(dataIndex,recordListIndex):
            # dataIndex is the box number for the Med Associates file
            # recordListIndex points to the dataRecord in the main program
            if verbose:
                print("Creating Data record for box", recordListIndex)
                print("Dose = ", self.neumaierDose.get())
                print("File Format =", fileFormat)
            if fileFormat == 1:
                # Convert Inter-Infusion Intervals into a list of infusion timestamps (in mSec):
                timeStampList = []
                time = 0
                for t in range(len(boxList[dataIndex])):
                    time = time + (boxList[dataIndex][t]*60000)         
                    timeStampList.append(int(time))
            elif fileFormat == 2:
                # Convert Response Times into a list of response timestamps (in mSec):
                timeStampList = []
                time = 0
                for t in range(len(boxList[dataIndex])):
                    time = (boxList[dataIndex][t]*10)         
                    timeStampList.append(int(time))
                              
            # print("timeStampList:", timeStampList)
            '''
            # Translate the infusion timespamps into "Standardized" timestamp list using
            Standardized Timestamp codes 
            B, b = Start and stop of a first lever block
            A, a = Start and stop of a second lever block
            L, l = First lever down and up 
            J, j = Second lever down and up
            ...
            '''
            duration = 4000                
            tempDataList = []               # create empty list
            time = 0
            pair = [time,"B"]
            tempDataList.append(pair)
            for dataPoint in timeStampList:
                time = dataPoint
                pair = [time,"P"]
                tempDataList.append(pair)
                pair = [time,"L"]
                ...
                tempDataList.append(pair)
            pair = [time,"b"]             # add an "End Block" time stamp
            tempDataList.append(pair)
            # print(tempDataList)
            timeStampList = []   # clear the list
            # Move the list into the main program
            self.recordList[dataIndex].recordListIndex = [] # empty the main program list
            name = fileName+"_Box "+ str(dataIndex+1)
            self.recordList[recordListIndex].fileName = name
            self.fileNameList[recordListIndex].set(name)
            # changed from 1.0, 2.0 and 4.0
            cocConc = 2.7                           # default 
            if self.neumaierDose.get() == 1: cocConc = 1.35
            if self.neumaierDose.get() == 3: cocConc = 5.4  
            self.recordList[recordListIndex].cocConc = cocConc    # mg/ml
            self.recordList[recordListIndex].pumpSpeed = 0.025 # ml/sec            
            self.recordList[recordListIndex].dataList = tempDataList
            self.recordList[recordListIndex].extractStatsFromList()
            if verbose: print(self.recordList[recordListIndex])
          
        boxInt = 0
        if boxNum == "All":
            print("Loading",boxNum)
            for dataIndex in range(8):
                createDataFile(dataIndex,dataIndex)
        else:
            boxInt = int(boxNum)
            print("Loading Box", boxInt)
            selected = self.fileChoice.get()
            createDataFile(boxInt-1,selected)           
            
        if verbose: print (" *********** Exiting loadWashUFile procedure *************")
